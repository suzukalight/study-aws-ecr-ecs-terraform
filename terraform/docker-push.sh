...
export ECR_URI_NGINX=$(aws ecr describe-repositories --repository-names ${PROJECT_NAME}-nginx --query 'repositories[0].repositoryUri' --output text)
docker build -t ${ECR_URI_NGINX} -f docker/nginx/Dockerfile .
docker push ${ECR_URI_NGINX}

export ECR_URI_BACKEND=$(aws ecr describe-repositories --repository-names ${PROJECT_NAME}-backend --query 'repositories[0].repositoryUri' --output text)
